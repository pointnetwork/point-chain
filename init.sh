...
pointd add-genesis-account $KEY 100000000000000000000000000aevmos --keyring-backend $KEYRING
...
## 3. Clone this ~/.pointd home directory into some others, let's say `~/.clonedEvmosd`
## 4. Run `gentx` in each of those folders
## 5. Copy the `gentx-*` folders under `~/.clonedEvmosd/config/gentx/` folders into the original `~/.pointd/config/gentx`
...
pointd start --pruning=nothing $TRACE --log_level $LOGLEVEL --minimum-gas-prices=0.001apoint --json-rpc.api eth,txpool,personal,net,debug,web3

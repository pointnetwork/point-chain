...
Example:
	pointd testnet init-files --v 4 --output-dir ./.testnets --starting-ip-address 192.168.10.2
	`,
	...
	cmd.Flags().String(flagNodeDaemonHome, "pointd", "Home directory of the node's daemon configuration")
...
Example:
	pointd testnet --v 4 --output-dir ./.testnets
	`,
		RunE: func(cmd *cobra.Command, _ []string) error {
			args := startArgs{}
			args.outputDir, _ = cmd.Flags().GetString(flagOutputDir)
...
func initTestnetFiles(
	...
	if args.chainID == "" {
		args.chainID = fmt.Sprintf("point_%d-1", tmrand.Int63n(9999999999999)+1)
	}
	...
	for i := 0; i < args.numValidators; i++ {
		...
		kb, err := keyring.New(sdk.KeyringServiceName(), args.keyringBackend, nodeDir, inBuf, clientCtx.Codec, pointkr.Option())
		if err != nil {
			return err
		}

...
build-docker:
	...
	mkdir -p ./build/
	$(DOCKER) cp evmos:/usr/bin/pointd ./build/
...
localnet-start: localnet-stop localnet-build
	@if ! [ -f build/node0/$(EVMOS_BINARY)/config/genesis.json ]; then docker run --rm -v $(CURDIR)/build:/evmos:Z evmos/node "./pointd testnet init-files --v 4 -o /evmos --keyring-backend=test --starting-ip-address 192.167.10.2"; fi
	docker-compose up -d
...
localnet-unsafe-reset:
	docker-compose down
ifeq ($(OS),Windows_NT)
	@docker run --rm -v $(CURDIR)\build\node0\pointd:/evmos\Z evmos/node "./pointd tendermint unsafe-reset-all --home=/evmos"
	@docker run --rm -v $(CURDIR)\build\node1\pointd:/evmos\Z evmos/node "./pointd tendermint unsafe-reset-all --home=/evmos"
	@docker run --rm -v $(CURDIR)\build\node2\pointd:/evmos\Z evmos/node "./pointd tendermint unsafe-reset-all --home=/evmos"
	@docker run --rm -v $(CURDIR)\build\node3\pointd:/evmos\Z evmos/node "./pointd tendermint unsafe-reset-all --home=/evmos"
else
	@docker run --rm -v $(CURDIR)/build/node0/pointd:/evmos:Z evmos/node "./pointd tendermint unsafe-reset-all --home=/evmos"
	@docker run --rm -v $(CURDIR)/build/node1/pointd:/evmos:Z evmos/node "./pointd tendermint unsafe-reset-all --home=/evmos"
	@docker run --rm -v $(CURDIR)/build/node2/pointd:/evmos:Z evmos/node "./pointd tendermint unsafe-reset-all --home=/evmos"
	@docker run --rm -v $(CURDIR)/build/node3/pointd:/evmos:Z evmos/node "./pointd tendermint unsafe-reset-all --home=/evmos"
endif

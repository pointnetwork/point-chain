...
ldflags = -X github.com/cosmos/cosmos-sdk/version.Name=point \
          -X github.com/cosmos/cosmos-sdk/version.AppName=$(POINT_BINARY) \
...
build-docker:
	...
	$(DOCKER) rm point || true
	# create a new container from the latest image
	$(DOCKER) create --name point -t -i ${DOCKER_IMAGE}:latest point
	# move the binaries to the ./build directory
	mkdir -p ./build/
	$(DOCKER) cp point:/usr/bin/pointd ./build/
...
godocs:
	@echo "--> Wait a few seconds and visit http://localhost:6060/pkg/github.com/pointnetwork/point-chain/types"
	godoc -http=:6060
...
test-sim-custom-genesis-fast:
	...
	@echo "By default, ${HOME}/.$(POINT_DIR)/config/genesis.json will be used."
	@go test -mod=readonly $(SIMAPP) -run TestFullAppSimulation -Genesis=${HOME}/.$(POINT_DIR)/config/genesis.json \
...
test-sim-custom-genesis-multi-seed: runsim
	...
	@echo "By default, ${HOME}/.$(POINT_DIR)/config/genesis.json will be used."
	@$(BINDIR)/runsim -Genesis=${HOME}/.$(POINT_DIR)/config/genesis.json -SimAppPkg=$(SIMAPP) -ExitOnFail 400 5 TestFullAppSimulation
...
localnet-start: localnet-stop localnet-build
	@if ! [ -f build/node0/$(POINT_BINARY)/config/genesis.json ]; then docker run --rm -v $(CURDIR)/build:/point:Z point/node "./pointd testnet init-files --v 4 -o /point --keyring-backend=test --starting-ip-address 192.167.10.2"; fi
	docker-compose up -d
...
localnet-unsafe-reset:
	docker-compose down
ifeq ($(OS),Windows_NT)
	@docker run --rm -v $(CURDIR)\build\node0\pointd:/point\Z point/node "./pointd tendermint unsafe-reset-all --home=/point"
	@docker run --rm -v $(CURDIR)\build\node1\pointd:/point\Z point/node "./pointd tendermint unsafe-reset-all --home=/point"
	@docker run --rm -v $(CURDIR)\build\node2\pointd:/point\Z point/node "./pointd tendermint unsafe-reset-all --home=/point"
	@docker run --rm -v $(CURDIR)\build\node3\pointd:/point\Z point/node "./pointd tendermint unsafe-reset-all --home=/point"
else
	@docker run --rm -v $(CURDIR)/build/node0/pointd:/point:Z point/node "./pointd tendermint unsafe-reset-all --home=/point"
	@docker run --rm -v $(CURDIR)/build/node1/pointd:/point:Z point/node "./pointd tendermint unsafe-reset-all --home=/point"
	@docker run --rm -v $(CURDIR)/build/node2/pointd:/point:Z point/node "./pointd tendermint unsafe-reset-all --home=/point"
	@docker run --rm -v $(CURDIR)/build/node3/pointd:/point:Z point/node "./pointd tendermint unsafe-reset-all --home=/point"
endif
...
PACKAGE_NAME:=github.com/pointnetwork/point-chain

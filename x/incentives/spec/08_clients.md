...
Find below a list of `pointd` commands added with the `x/incentives` module. You can obtain the full list by using the `pointd -h` command.
...
```go
pointd query incentives incentives [flags]
```
...
```go
pointd query incentives incentive [contract-address] [flags]
```
...
```bash
pointd query incentives gas-meters [contract-address] [flags]
```
...
```go
pointd query incentives gas-meter [contract-address] [participant-address] [flags]
```
...
```bash
pointd query incentives params [flags]
```
...
```bash
pointd tx gov submit-proposal register-incentive [contract-address] [allocation] [epochs] [flags]
```
...
```bash
pointd tx gov submit-proposal cancel-incentive [contract-address] [flags]
```

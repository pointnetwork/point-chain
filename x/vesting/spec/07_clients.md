...
Find below a list of `pointd` commands added with the `x/vesting` module. You can obtain the full list by using the `pointd -h` command.
...
```go
pointd add-genesis-account [address_or_key_name] [coin][,[coin]] [flags]
```
...
```go
pointd query vesting balances [address] [flags]
```
...
```go
pointd tx vesting create-clawback-vesting-account [to_address] [flags]
```
...
```go
pointd tx vesting clawback [address] [flags]
```

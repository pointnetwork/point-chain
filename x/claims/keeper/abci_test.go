...
func (suite *KeeperTestSuite) TestClawbackEmptyAccounts() {
	...
	testCases := []struct {
		...
		},
		{
			...
			func() {
				...
				coins := sdk.NewCoins(sdk.NewCoin("apoint", sdk.NewInt(amount)))
	...
	for _, tc := range testCases {
		suite.Run(fmt.Sprintf("Case %s", tc.name), func() {
			...
			suite.app.ClaimsKeeper.ClawbackEmptyAccounts(suite.ctx, "apoint")

			moduleAcc := suite.app.AccountKeeper.GetModuleAccount(suite.ctx, distrtypes.ModuleName)
			balance := suite.app.BankKeeper.GetBalance(suite.ctx, moduleAcc.GetAddress(), "apoint")
...
func (suite *KeeperTestSuite) TestClawbackEscrowedTokensABCI() {
	...
	for _, tc := range testCases {
		suite.Run(fmt.Sprintf("Case %s", tc.name), func() {
			...
			balance := suite.app.BankKeeper.GetBalance(suite.ctx, acc.GetAddress(), "apoint")

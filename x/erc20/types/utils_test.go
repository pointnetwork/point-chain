	...
	testCases := []struct {
	...
	}{
		{
			...
			banktypes.Metadata{
				...
				Name:        "Point",
				Symbol:      "POINT",
				Description: "EVM, staking and governance denom of Point",
				DenomUnits: []*banktypes.DenomUnit{
					{
						Denom:    "apoint",
						Exponent: 0,
						Aliases:  []string{"atto point"},
					},
					{
						Denom:    "point",
						Exponent: 18,
...
func TestEqualAliases(t *testing.T) {
	testCases := []struct {
		...
		},
		{
			...
			[]string{"atto point", "apoint"},
			[]string{"apoint", "atto point"},
			false,
		},
		{
			"same values, sorted",

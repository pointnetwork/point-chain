...
Find below a list of  `pointd` commands added with the  `x/erc20` module. You can obtain the full list by using the `pointd -h` command. A CLI command can look like this:

```bash
pointd query erc20 params
```
...
```bash
pointd tx gov submit-proposal register-coin [metadata] [flags]
```
...
```bash
pointd tx gov submit-proposal register-erc20 [erc20-address] [flags]
```
...
```bash
pointd tx gov submit-proposal toggle-token-conversion [token] [flags]
```
